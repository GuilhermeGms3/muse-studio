...
export function MetronomePage() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <Breadcrumb trail={["Praticar", "Metrônomo"]} />
          ...
          <div className="mt-3 flex gap-px">
            {[-8, -4, -1, 1, 4, 8].map((d) => (
              <button
                key={d}
                onClick={() =>
                  setMetronome({ bpm: Math.max(30, Math.min(260, metronome.bpm + d)) })
                }
          ...
          <Panel title="Presets">
            {presets.map((p) => (
              <button
                key={p.name}
                onClick={() =>
                  setMetronome({ bpm: p.bpm, beats: p.beats, subdivision: p.subdivision })
                }
