...
import { Panel, StateDot, StateTag, Meter } from "@/components/workspace/Panel";
...
export const Route = createFileRoute("/skills")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Mapa progressivo do conhecimento e das habilidades musicais." },
    ],
  }),
...
function SkillsPage() {
  ...
  const relatedLibrary = (libraryQuery.data ?? []).filter((item) => selected.contents.includes(item.id));
  ...
  const relatedSongs = (songsQuery.data ?? []).filter(
    (song) => selected.songs.includes(song.id)
            ...
            <div className="flex items-center gap-3">
              <select
                ...
                onChange={(event) => {
                  ...
                  const first = skills.find((skill) => skill.domain === domain);
              ...
              <div className="hidden items-center gap-2 xl:flex">
                {Object.entries(stateText).map(([key, label]) => (
                  <span key={key} className="flex items-center gap-1 text-2xs text-muted-foreground">
        ...
        >
          <div className="h-full w-full overflow-auto bg-rail">
            <svg width={svgWidth} height={svgHeight} viewBox={`0 0 ${svgWidth} ${svgHeight}`}>
              <text x="54" y="27" fill="var(--color-muted-foreground)" fontFamily="var(--font-mono)" fontSize="10">
              ...
              {layout.map((node) => {
                ...
                return (
                  <g key={node.skill.id} className="cursor-pointer" onClick={() => selectSkill(node.skill.id)}>
                    <title>{node.skill.friendlyTitle} - {node.skill.technicalName}</title>
                    <rect
                      x={node.x}
                      y={node.y}
                      width="184"
        ...
        <aside className="flex min-h-0 flex-col gap-px overflow-auto bg-border">
          <Panel title="Habilidade">
            ...
            <div className="mt-3 flex items-center gap-3">
              <Meter value={selected.progress} tone={selected.state === "mastered" ? "ok" : "info"} />
            ...
            {selected.nextReviewAt && (
              <div className="mt-1 flex justify-between text-2xs text-muted-foreground">
                ...
                <span className="num">{new Date(selected.nextReviewAt).toLocaleDateString("pt-BR")}</span>
              </div>
            )}
            <button
          ...
          <Panel title="Próximo critério">
            {selected.nextRequirements.map((requirement) => (
              <p key={requirement} className="border-l border-signal pl-2 text-xs text-muted-foreground">
                {requirement}
              </p>
            ))}
          </Panel>
          ...
          <Panel title="Pré-requisitos">
            {!selected.prerequisites.length && <p className="text-2xs text-muted-foreground">Nenhum.</p>}
            {selected.prerequisites.map((id) => (
              <button key={id} onClick={() => selectSkill(id)} className="flex items-center gap-2 text-xs hover:text-signal">
          ...
          <Panel title="Conteúdos">
            ...
            {relatedLibrary.map((item) => (
              <Link key={item.id} to="/biblioteca/$nodeId" params={{ nodeId: item.id }} className="block text-xs hover:text-signal">
                {item.friendlyTitle}
          ...
          <Panel title="Exercícios">
            {!relatedExercises.length && <p className="text-2xs text-muted-foreground">Nenhum exercício ligado.</p>}
          ...
          <Panel title="Músicas relacionadas">
            {!relatedSongs.length && <p className="text-2xs text-muted-foreground">Nenhuma música ligada.</p>}
            {relatedSongs.map((song) => (
              <Link key={song.id} to="/repertorio/$songId" params={{ songId: song.id }} className="block text-xs hover:text-signal">
                {song.title}
              </Link>
            ))}
          </Panel>
          ...
          <Panel title="Próximas habilidades">
            {!selected.nextSkills.length && <p className="text-2xs text-muted-foreground">Fim deste ramo.</p>}
