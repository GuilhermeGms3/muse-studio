        ...
        <div>
          ...
          {project.riffs.map((riff) => (
            <div key={riff.id} className="mt-1 border border-border bg-rail p-2">
              ...
              <pre className="num mt-1 overflow-x-auto text-2xs text-muted-foreground">{riff.tab}</pre>
            </div>
          ))}
        </div>
        <div>
          ...
          {project.ideas.map((idea) => (
            <div key={idea} className="mt-1 border-l border-signal pl-2 text-xs text-muted-foreground">{idea}</div>
          ))}
          ...
          {project.versions.map((version) => (
            <div key={version.id} className="flex justify-between text-xs text-muted-foreground">
              <span>{version.label}</span><span className="num">{version.date}</span>
            </div>
          ))}
          <span className="label-tech mt-4 block">Referências</span>
          {project.references.map((reference) => (
            <div key={reference} className="text-xs text-muted-foreground">· {reference}</div>
          ))}
        </div>
      </div>
    </div>
